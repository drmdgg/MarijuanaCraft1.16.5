...
public class ColumnTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public final ItemStackHandler inventory = new ItemStackHandler(3) {
		...
		protected void onContentsChanged(final int slot) {
			...
			ColumnTileEntity.this.markDirty();
	...
	public ColumnTileEntity() {

		super(ModTileEntityTypes.COLUMN.get());
	...
	public void tick() {
		...
		if (lastBurning != this.isBurning()) {
			...
			final BlockState newState = BlockInit.COLUMNBLOCK.get().getDefaultState()

					.with(ColumnBlock.BURNING, this.isBurning());
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent(BlockInit.COLUMNBLOCK.get().getTranslationKey());
	...
	public Container createMenu(final int windowId, final PlayerInventory inventory, final PlayerEntity player) {
		...
		return new ColumnContainer(windowId, inventory, this);
